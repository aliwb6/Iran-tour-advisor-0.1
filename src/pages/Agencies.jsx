...
const TOUR_TYPE_OPTIONS = [
  ...
  { key: 'business',    en: 'Business',           fa: 'تجاری',           ar: 'أعمال' },
];

const DEFAULT_FILTERS = { city: 'all', language: 'all', tourType: 'all' };

// ── Agency Card — PRESERVED EXACTLY AS-IS ────────────────────────────────────
...
export default function Agencies() {
  ...
  const { agencies: rawAgencies, loading, error } = useAgencies();

  // Only show approved/licensed agencies
  const agencies = rawAgencies.filter(a => a.is_approved || a.is_licensed || a.is_verified !== false);
  ...
  const filtered = useMemo(() => {
    return agencies.filter(a => {
      const cityVal = (a.city || '').toLowerCase();
      const nameVal = (a.full_name || '').toLowerCase();
      const bioVal = (a.bio || '').toLowerCase();

      if (search && !nameVal.includes(search.toLowerCase()) &&
          !cityVal.includes(search.toLowerCase()) &&
          !bioVal.includes(search.toLowerCase())) return false;

      if (filters.city !== 'all' && !cityVal.includes(filters.city.toLowerCase())) return false;

      if (filters.language !== 'all') {
        const langs = Array.isArray(a.languages ?? a.support_languages)
          ? (a.languages ?? a.support_languages).map(l => String(l).toLowerCase())
          : String(a.languages ?? a.support_languages ?? '').toLowerCase().split(/[,;·]/).map(s => s.trim());
        if (!langs.some(l => l.includes(filters.language.toLowerCase()))) return false;
      }

      if (filters.tourType !== 'all') {
        const types = Array.isArray(a.tour_types ?? a.specialties)
          ? (a.tour_types ?? a.specialties).map(s => String(s).toLowerCase())
          : [String(a.specialty ?? '').toLowerCase()];
