...
const TOUR_TYPE_OPTIONS = [
  { key: 'all',         en: 'All Tour Types',     fa: 'همه انواع تور',   ar: 'كل أنواع الجولات' },
  ...
  { key: 'budget',      en: 'Budget-friendly',     fa: 'اقتصادی',          ar: 'اقتصادي' },
  { key: 'photography', en: 'Photography',         fa: 'عکاسی',            ar: 'تصوير' },
  { key: 'religious',   en: 'Religious & Ziyarat', fa: 'مذهبی و زیارتی',  ar: 'ديني وزيارة' },
  { key: 'nature',      en: 'Nature & Eco',        fa: 'طبیعت‌گردی',       ar: 'طبيعة وبيئة' },
  { key: 'academic',    en: 'Academic / Research', fa: 'علمی و پژوهشی',    ar: 'أكاديمي وبحثي' },
];
...
const DEFAULT_FILTERS = { search: '', city: 'all', tourType: 'all', language: 'all' };
...
function rowMatches(a, f) {
  ...
  if (f.city !== 'all') {
    ...
    if (!city.includes(f.city.toLowerCase())) return false;
  }
  if (f.tourType !== 'all') {
    const types = parseList(a.tour_types ?? a.specialties);
    if (types.length > 0 && !types.some(t => t.includes(f.tourType.toLowerCase()))) return false;
  }
  if (f.language !== 'all') {
    const langs = parseList(a.languages ?? a.support_languages);
    if (langs.length > 0 && !langs.some(l => l.includes(f.language.toLowerCase()))) return false;
  }
  return true;
}

// ── Agency Card — DO NOT MODIFY ───────────────────────────────────────────────
...
export default function Agencies() {
  ...
  // Only show approved/licensed agencies
  const approvedAgencies = useMemo(
    () => agencies.filter(a => a.is_approved || a.is_verified || a.is_licensed),
    [agencies]
  );

  const hasActive = filters.city !== 'all' || filters.tourType !== 'all' || filters.language !== 'all';
  const reset = () => setFilters(DEFAULT_FILTERS);

  const filtered = useMemo(
    () => approvedAgencies.filter((a) => rowMatches(a, filters)),
    [approvedAgencies, filters]
  );
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-10">
          ...
          <div className="relative max-w-lg">
            <Search className={`absolute top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground pointer-events-none ${dir === 'rtl' ? 'end-4' : 'start-4'}`} />
            <input
              type="text"
              value={filters.search}
              onChange={(e) => setFilters(f => ({ ...f, search: e.target.value }))}
              placeholder={lang === 'fa' ? 'جستجو در آژانس‌ها...' : lang === 'ar' ? 'ابحث في الوكالات...' : 'Search agencies by name, city, tour type...'}
              className={`w-full ${dir === 'rtl' ? 'pe-11 ps-4' : 'ps-11 pe-4'} py-3 rounded-2xl border border-border bg-card font-body text-sm text-foreground placeholder:text-muted-foreground/60 focus:outline-none focus:ring-2 focus:ring-gold/40 focus:border-gold/40 transition shadow-sm`}
            />
          </div>
        </motion.div>
