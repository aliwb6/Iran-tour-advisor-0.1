...
function guideMatches(g, search, filters) {
  if (search) {
  ...
  if (filters.city !== 'all') {
    if (!(g.city || '').toLowerCase().includes(filters.city.toLowerCase())) return false;
  }
  ...
  if (filters.specialty !== 'all') {
    const specs = [
      ...parseList(g.specialties),
      ...(g.specialty ? [String(g.specialty).toLowerCase()] : []),
    ];
    if (specs.length > 0 && !specs.some(s => s.includes(filters.specialty.toLowerCase()))) return false;
  }
  return true;
}

// ── Guide Card — DO NOT MODIFY THIS COMPONENT ─────────────────────────────────
...
export default function Guides() {
  ...
  // Prefer Supabase (hook already filters is_approved: true); fall back to local verified guides
  ...
  const filtered = useMemo(
    () => allGuides.filter(g => guideMatches(g, search, filters)),
  ...
  return (
    <div dir={dir} className="pt-24 pb-16">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Dropdown filter bar — same pattern as TourFilters */}
        <div className="mb-10">
